...
# ║╚╔═╗╝║  │┌┘─└┐│  ▄█▀‾
# ============================= #
# Database Guillotine MGCP v1.0 #
#      Nat Cagle 2022-07-25     #
# ============================= #
import arcpy as ap
...
#######
Use existing GDB doesn't work on SDE because the feature class names are sde_name.sde.featureclass
#######
	...
	for fc in fc_list:
		...
		with ap.da.SearchCursor(AOI, ['SHAPE@']) as aoi_cur:
			...
			if 'MetadataSrf' in fc or 'ResourceSrf' in fc: # Check for these first since they are a special case
				...
				try:
					with ap.da.SearchCursor(input_fc, field_list) as input_scursor: # Search the input feature class with the specified fields and user defined query
						try:
